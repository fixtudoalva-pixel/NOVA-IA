-- Bound untrusted JSON persisted by the Operator commit RPC.
...
begin
 ...
 for v_action in select value from jsonb_array_elements(p_actions) loop
  ...
  values(v_org,p_conversation_id,v_action->>'type',case when v_requires then 'awaiting_approval'::public.action_status else 'approved'::public.action_status end,v_risk,left(coalesce(v_action->>'rationale',''),2000),coalesce(v_action->'payload','{}'::jsonb)||jsonb_build_object('agent_run_id',v_run)) returning id into v_action_id;
