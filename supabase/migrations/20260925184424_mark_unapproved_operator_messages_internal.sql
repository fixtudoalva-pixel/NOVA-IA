CREATE OR REPLACE FUNCTION public.commit_operator_decision(p_conversation_id uuid, p_source_message_id uuid, p_decision jsonb, p_actions jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
...
begin
 ...
 values(v_org,p_conversation_id,'internal','agent',left(coalesce(p_decision->>'reply',''),10000),jsonb_build_object('confidence',p_decision->'confidence','intent',left(coalesce(p_decision->>'intent','unknown'),120),'agent_run_id',v_run));
 return v_run;
end $function$
;
update public.messages set direction='internal' where conversation_id='9002be44-ebdb-4644-8531-115a22376e61'::uuid and actor='agent' and direction='outbound' and metadata ? 'agent_run_id'
