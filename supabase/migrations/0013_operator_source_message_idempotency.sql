-- Use the immutable source message id as the Operator idempotency key.
alter table public.agent_runs add column if not exists source_message_id uuid references public.messages(id) on delete set null;
drop index if exists public.agent_runs_one_completed_per_message_idx;
create unique index if not exists agent_runs_one_completed_per_source_message_idx
on public.agent_runs(conversation_id,source_message_id)
where status='completed' and source_message_id is not null;
