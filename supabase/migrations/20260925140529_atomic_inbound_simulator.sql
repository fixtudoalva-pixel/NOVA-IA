...
begin
 ...
 if length(trim(coalesce(p_body,'')))=0 then raise exception 'message required'; end if;
 ...
 values(v_org,nullif(trim(coalesce(p_name,'')),'')) returning id into v_contact;
 insert into public.conversations(organization_id,contact_id,channel)
 values(v_org,v_contact,'simulator') returning id into v_conversation;
 insert into public.messages(organization_id,conversation_id,direction,actor,body)
 values(v_org,v_conversation,'inbound','customer',trim(p_body));
