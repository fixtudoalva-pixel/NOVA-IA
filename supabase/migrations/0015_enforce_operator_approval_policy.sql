-- Harden the Operator RPC so approval requirements are derived server-side for MVP autonomy level 2.
...
begin
 ...
 for v_action in select value from jsonb_array_elements(p_actions) loop
  ...
  -- MVP Operator runs at autonomy level 2: every external side effect requires human approval.
  v_requires=true;
