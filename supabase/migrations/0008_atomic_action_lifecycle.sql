...
begin
  ...
  where id=p_action_id and status='approved' and public.is_org_member(organization_id) for update;
  if not found then return; end if;
  update public.actions set status='completed', completed_at=v_completed,
    output=jsonb_build_object('executor','simulator','completed_at',v_completed) where id=v_action.id;
...
begin
 ...
 where id=p_approval_id and action_id=p_action_id and decision is null and public.is_org_member(organization_id) for update;
 if not found then return; end if;
