-- Harden the persistence boundary: validate JSON shape, block critical actions and force approval for high risk.
...
declare v_org uuid; v_run uuid; v_action jsonb; v_action_id uuid; v_requires boolean; v_risk text;
begin
 ...
 select organization_id into v_org from public.conversations where id=p_conversation_id and public.is_org_member(organization_id) for update;
 ...
 for v_action in select value from jsonb_array_elements(p_actions) loop
  ...
  v_requires=case when v_risk='high' then true else coalesce((v_action->>'requiresApproval')::boolean,true) end;
  insert into public.actions(organization_id,conversation_id,action_type,status,risk,rationale,input)
  values(v_org,p_conversation_id,v_action->>'type',case when v_requires then 'awaiting_approval'::public.action_status else 'approved'::public.action_status end,v_risk,v_action->>'rationale',coalesce(v_action->'payload','{}'::jsonb)||jsonb_build_object('agent_run_id',v_run)) returning id into v_action_id;
 ...
 values(v_org,p_conversation_id,'outbound','agent',left(coalesce(p_decision->>'reply',''),10000),jsonb_build_object('confidence',p_decision->'confidence','intent',p_decision->'intent','agent_run_id',v_run));
 return v_run;
end $$;
