  ...
  return <main>
    ...
    <section className="card ledger" aria-labelledby="knowledge-list-heading">
      ...
      {items?.map(item => <div className="row" key={item.id}>
        ...
        <div>{!item.is_approved && <form aria-label={`Aprovar conhecimento ${item.title}`}><input type="hidden" name="id" value={item.id}/><button formAction={approveKnowledge} type="submit" aria-label={`Aprovar conhecimento: ${item.title}`}>Aprovar</button></form>}</div>
      </div>)}
    </section>
  </main>;
}
