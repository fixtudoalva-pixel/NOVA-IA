...
export async function addKnowledge(formData: FormData) {
  ...
  if (title.length < 2 || title.length > 160 || content.length < 2 || content.length > 10000 || /[\u0000-\u0008\u000B\u000C\u000E-\u001F]/.test(title + content) || !allowedKinds.has(kind)) {
...
export async function approveKnowledge(formData: FormData) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(id)) redirect("/knowledge?error=approval");
