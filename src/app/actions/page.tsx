    ...
    <section className="card ledger"><h2>Ledger</h2>
      ...
      {actions?.map(a => {
        ...
        return <div className="row" key={a.id}>
          ...
          {a.status==="completed" && <form className="inline-form"><input type="hidden" name="action_id" value={a.id}/><input name="amount" type="number" min="0.01" max="100000000" step="0.01" placeholder={orgSettings?.currency ?? "EUR"} defaultValue={revenue ? (revenue/100).toFixed(2) : undefined}/><button formAction={recordSaleOutcome}>{revenue ? "Atualizar venda" : "Registar venda"}</button></form>}</div>
        </div>;
      })}
    </section>
  </main>;
