    ...
    <section className="card ledger" aria-labelledby="ledger-heading"><h2 id="ledger-heading">Ledger</h2>
      ...
      {actions?.map(a => {
        ...
        return <div className="row" key={a.id}>
          ...
          {a.status==="completed" && <form aria-label={`Registar resultado comercial de ${a.action_type}`} className="inline-form"><input type="hidden" name="action_id" value={a.id}/><input name="amount" aria-label="Valor da venda" aria-describedby="sale-help" type="number" min="0.01" max="100000000" step="0.01" inputMode="decimal" placeholder={orgSettings?.currency ?? "EUR"} defaultValue={revenue ? (revenue/100).toFixed(2) : undefined}/><button formAction={recordSaleOutcome} type="submit" aria-label={`${revenue ? "Atualizar" : "Registar"} venda para ${a.action_type}`}>{revenue ? "Atualizar venda" : "Registar venda"}</button></form>}</div>
        </div>;
      })}
    <p id="sale-help">Regista apenas uma venda confirmada por uma pessoa. O valor será tratado como receita assistida, não causal.</p></section>
  </main>;
}