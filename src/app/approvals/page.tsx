    ...
    <section className="card ledger" aria-labelledby="approvals-heading">
      ...
      {approvals?.map(a => {
        ...
        return <div className="row" key={a.id}>
          ...
          <div>Risco declarado: {action.risk}</div><div>{action.status.replaceAll("_", " ")}</div>
          <div className="auth-actions"><form aria-label={`Decisão humana para ${action.action_type}`}><input type="hidden" name="approval_id" value={a.id}/><input type="hidden" name="action_id" value={action.id}/><button formAction={approve} type="submit" aria-label={`Aprovar ${action.action_type}`}>Aprovar</button><button className="secondary" formAction={reject} type="submit" aria-label={`Rejeitar ${action.action_type}`}>Rejeitar</button></form></div>
        </div>;
      })}
    </section>
  </main>;
